...
### 2) Modularity
The entire launcher subsystem comprises of [**HOW MANY**] components instead of being modelled as a single piece. This allows iterative design improvements on individual components instead of having to re-print the whole launcher.

All components are assembled together using 3 primary methods:
...
### 3) Adjustability
Building on the modular design aspect, the launcher assembly features some adjustable components. The ball exit hood, for example, can be adjusted vertically to accomodate different rubber band thicknesses or the amount of flywheel contact.
...
Most of the components in the flywheel assembly requires little to no support for 3D printing, drastically increasing the ease of production and decreasing print time (time = money). The components are designed with a ground-up approach: in the prelimary design stages of each component, a single plane is established as the "ground plane" where every other geometrical feature strictly builds up from. Such a design philosophy ensures that the components do not contain any overhangs/floating features when printed from the ground plane up, eliminating supports.

### 5) Bearings
Bearings are integrated into the design to constrain rotational motion and enhance flywheel smoothness.
